...
Short description:
Compare JSON API responses across Dev, QA, and Prod from a Microsoft Edge popup.

Detailed description:
API Response Comparator helps developers and QA teams spot API response drift before it reaches users. Paste JSON, upload response files, fetch HTTPS API URLs, or import safe cURL commands, then compare Dev, QA, and Prod responses from a compact Microsoft Edge extension popup.
...
`storage`:
Stores draft inputs and UI preferences locally in Edge extension storage so users do not lose in-progress comparisons when the popup closes.
...
1. Load the extension in Microsoft Edge and open the toolbar popup.
...
Current package behavior intentionally differs from the Next.js app proxy: the Edge extension performs browser extension fetches directly using HTTPS host permissions. Browser-restricted headers are filtered from cURL imports before fetch.
